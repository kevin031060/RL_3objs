...
def reward(static, tour_indices, w1=1, w2=0, w3=0):
    """
    Parameters
    ----------
    ...
    obj2 = torch.abs(y_obj2[:, :-1] - y_obj2[:, 1:]).sum(1).detach()
    obj3 = torch.abs(y_obj3[:, :-1] - y_obj3[:, 1:]).sum(1).detach()

    obj = w1*obj1 + w2*obj2 + w3*obj3
