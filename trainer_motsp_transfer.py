...
def validate(data_loader, actor, reward_fn, w1, w2, w3, render_fn=None, save_dir='.',
             num_plot=5):
    ...
    for batch_idx, batch in enumerate(data_loader):
        ...
        reward, obj1, obj2, obj3 = reward_fn(static, tour_indices, w1, w2, w3)

        rewards.append(torch.mean(reward.detach()).item())
        obj1s.append(torch.mean(obj1.detach()).item())
        obj2s.append(torch.mean(obj2.detach()).item())
        obj3s.append(torch.mean(obj2.detach()).item())
...
def train(actor, critic, w1, w2, w3, task, num_nodes, train_data, valid_data, reward_fn,
    ...
    now = now.replace(':', '_')
    bname = "_transfer"
    save_dir = os.path.join(task+bname, '%d' % num_nodes, 'w_%2.2f_%2.2f_%2.2f' % (w1, w2, w3), now)
    ...
    for epoch in range(5):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            if (batch_idx + 1) % 200 == 0:
                ...
                print('  Batch %d/%d, reward: %2.3f, obj1: %2.3f, obj2: %2.3f, obj3: %2.3f, loss: %2.4f, took: %2.4fs' %
                      (batch_idx, len(train_loader), mean_reward, mean_obj1, mean_obj2, mean_obj3, mean_loss,
                       times[-1]))
        ...
        mean_valid, mean_obj1_valid, mean_obj2_valid, mean_obj3_valid = validate(valid_loader, actor, reward_fn, w1, w2, w3, render_fn,
                              '.', num_plot=5)
        ...
        if mean_valid < best_reward:
            ...
            main_dir = os.path.join(task+bname, '%d' % num_nodes, 'w_%2.2f_%2.2f_%2.2f' % (w1, w2, w3))
...
def train_tsp(args, w1=1, w2=0, w3=0, checkpoint = None):
    ...
    if not args.test:
        train(actor, critic, w1, w2, w3, **kwargs)
    ...
    out = validate(test_loader, actor, motsp.reward, w1, w2, w3, motsp.render, test_dir, num_plot=5)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train-size',default=400000, type=int)
    ...
    if args.task == 'tsp':
        for i in range(0,T+1):
            print("Current w:%2.2f/%2.2f/%2.2f"% (w2_list[i,0],w2_list[i,1],w2_list[i,2]))
            if i==0:
                # checkpoint = 'modified_checkpoint_3obj'
                checkpoint = 'tsp_transfer/40/w_1.00_0.00_0.00'
                train_tsp(args, 1, 0, 0, None)
            else:
                checkpoint = 'tsp_transfer/%d/w_%2.2f_%2.2f_%2.2f'%(num_nodes, w2_list[i-1,0], w2_list[i-1,1], w2_list[i-1,2])
                train_tsp(args, w2_list[i,0],w2_list[i,1],w2_list[i,2], checkpoint)
